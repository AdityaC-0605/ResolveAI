    ...
    <div className="px-8 py-8 max-w-5xl">
      ...
      <div className="glass mb-6 overflow-hidden">
            ...
            {inputs.map((row, i) => (
              ...
              >
                ...
                <button onClick={() => removeRow(row.id)} disabled={inputs.length === 1}
                  className="opacity-50 group-hover:opacity-100 transition-opacity text-slate-600 hover:text-coral shrink-0 disabled:opacity-20 disabled:hover:text-slate-600">
        ...
        <button onClick={addRow} disabled={inputs.length >= 50}
          className="w-full py-2.5 text-sm text-slate-500 hover:text-slate-300 hover:bg-white/[0.02] transition-colors flex items-center justify-center gap-2 disabled:opacity-30">
          <Plus className="w-4 h-4" /> Add complaint {inputs.length < 50 ? `(${50-inputs.length} remaining)` : '(max reached)'}
        </button>
      </div>

      {error && (
        <div className="glass p-4 mb-4" style={{ borderColor: 'rgba(255,45,107,0.2)', background: 'rgba(255,45,107,0.04)' }}>
          <p className="text-sm text-coral font-mono">{error}</p>
        </div>
      )}
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="glass overflow-hidden">
            <div className="px-6 py-4 border-b border-white/[0.04] flex items-center justify-between">
              ...
              <div className="flex gap-3 text-xs font-mono">
                <span className="text-mint">{results.successful} ok</span>
                <span className="text-coral">{results.failed} failed</span>
                    ...
                    <tr key={i}>
                      ...
                      {r.classification ? (
                        <>
                          <td>
                            <div>
                              <p className="text-sm text-slate-200 font-medium">{r.classification.category}</p>
                          ...
                          <td><CheckCircle2 className="w-4 h-4 text-mint" /></td>
                        </>
                      ) : (
                        <td colSpan={6} className="text-xs text-coral font-mono">
                          {r.error || 'Failed'}
                        </td>
                      )}
                    </tr>
