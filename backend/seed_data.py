"""
seed_data.py — Idempotent knowledge base initialisation.

Safe to run multiple times: documents are only inserted when their ID is absent
from the collection, so restarting the server never duplicates data.
"""
...
COMPLAINT_SEED_DATA = [
    ...
    {
        "id": "comp_013",
        "text": "My order has been stuck in 'processing' for 12 days. The tracking number shows no movement. I've emailed three times and received no reply.",
        "metadata": {"category": "Shipping", "subcategory": "stuck-order", "urgency": "medium"},
    },
    {
        "id": "comp_014",
        "text": "The software license I purchased isn't activating. Error code: ACTIVATION_FAILED_1047. I need this for a client presentation in 2 hours.",
        "metadata": {"category": "Technical", "subcategory": "license-activation", "urgency": "critical"},
    },
    {
        "id": "comp_015",
        "text": "I've been billed for a plan I never signed up for. I only agreed to the free tier. Please cancel and refund the last 3 months immediately.",
        "metadata": {"category": "Billing", "subcategory": "unauthorized-charge", "urgency": "high"},
    },
]


def seed_database(retriever: HybridRetriever | None = None) -> HybridRetriever:
    ...
    # add_documents is now idempotent — it skips existing IDs
    retriever.add_documents(texts, ids, metadatas)
    logger.info("Seed complete — collection contains %d documents", retriever.collection.count())
    return retriever
